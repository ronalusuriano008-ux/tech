...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (res && (res.success || res.queued)) {
      showToast(res.queued ? 'Entrada guardada localmente; se sincronizará al reconectar' : 'Entrada guardada');
      deleteDayButton.classList.remove('d-none');
      await fetchRawJson();
    } else {
...
async function deleteCurrentDay() {
  ...
  try {
    ...
    if (res && (res.success || res.queued)) {
      showToast(res.queued ? 'Eliminación pendiente de sincronización' : 'Día eliminado');
...
async function fetchRawJson() {
  try {
    ...
    const res = await fetch(diaryUrl, { credentials: 'include' });
