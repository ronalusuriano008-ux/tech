...
const validateSession = async () => {
    ...
    try {
        const res = await fetch(`${window.AppConfig?.apiBaseUrl || '/api'}/auth/check`, { credentials: 'include' });
...
const loadConfig = async () => {
    try {
        const res = await fetch(`${window.AppConfig?.apiBaseUrl || '/api'}/config`, { headers, credentials: 'include' });
        if (!res.ok) {
