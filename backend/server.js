...
// Importar almacenamiento de sesiones en disco
...
// Necesario para que Express respete X-Forwarded-Proto
// detrás de Cloudflare / Railway / proxy.
...
const authenticationRoot = path.join(
    frontendRoot,
    'modules',
    'authentication'
);

const adminPanelRoot = path.join(
    frontendRoot,
    'modules',
    'admin-panel'
);

const registroServiciosRoot = path.join(
    frontendRoot,
    'modules',
    'registro-servicios'
);

const calculadoraRoot = path.join(
    frontendRoot,
    'modules',
    'servicio-tecnico',
    'calculadora'
...
const servicioTecnicoPagesRoot = path.join(
    frontendRoot,
    'modules',
    'servicio-tecnico',
    'pages'
...
const servicioTecnicoJsRoot = path.join(
    frontendRoot,
    'modules',
    'servicio-tecnico',
    'js'
...
const tiendaPagesRoot = path.join(
    frontendRoot,
    'modules',
    'tienda',
    'pages'
...
// Ajustar cookie según HTTPS real detrás del proxy
...
const allowedOrigins = (
    ...
    [
        'http://localhost:3000',
        'http://127.0.0.1:3000',
        'https://tiendas.tumovil.xyz',
        'https://panel.vixbox.xyz',
        'https://beta.vixbox.xyz',
        'https://api.vixbox.xyz',
        'https://unmended-lacey-nondefinitively.ngrok-free.dev'
    ].join(',')
)
    .split(',')
...
const isAllowedTumovilSubdomain = (origin) => {
    try {
...
app.use(cors({
    origin(origin, callback) {
        ...
        if (
            ...
            isAllowedTumovilSubdomain(normalizedOrigin) ||
            isAllowedNgrokSubdomain(normalizedOrigin)
        ) {
...
app.use(express.json({
    limit: appConfig.http.jsonBodyLimit
}));
...
app.use((req, res, next) => {
    ...
    if (
        req.method === 'GET' &&
        protectedHtmlPaths.includes(req.path)
    ) {
        return next();
    }
...
app.use(async (req, res, next) => {
    ...
    try {
        ...
        if (protectedHtmlPaths.includes(req.path)) {
            return next();
        }
            ...
            html = html.replace(
                ...
                (m, g1) => {
                    ...
                    if (
                        ...
                        return `<html class="$1 glass-theme"${g1.replace(
                            /class=["'](.*?)"/,
                            ''
                        )}>`;
                    }
        ...
        console.error(
            '[theme-middleware] Error procesando HTML:',
            err.message
        );
...
app.get(
    ...
    (req, res) => {
        ...
        return res.status(404).json({
            error: 'stdiario.json no encontrado'
        });
    }
);
...
app.get(
    ...
    (req, res) => {
        ...
        res.json({
            status: 'ok',
            service: appConfig.app.serviceName,
...
// Compatibilidad para login.html
app.get(
    '/login.html',
    (req, res) => {
...
// Estas permiten que URLs antiguas sigan funcionando.
// ─────────────────────────────────────────────
...
const gracefulShutdown = (signal) => {
    ...
    server.close(() => {
        ...
        console.log(
            '[Interrupción] Servidor cerrado correctamente.'
        );
    ...
    setTimeout(() => {
        ...
        console.error(
            '[Interrupción] Forzando cierre por timeout.'
        );
...
process.on(
    ...
    (err) => {
        ...
        console.error(
            '[Error Crítico] Excepción no capturada:',
            err.message
        );
...
process.on(
    ...
    (reason) => {
        ...
        console.error(
            '[Error Crítico] Promesa rechazada no manejada:',
            reason
        );
